...
// so3g_exception is our internal base class.

class so3g_exception : std::exception {
    public:
        so3g_exception() {};

        so3g_exception(const char* txt) : text(txt) {}
...
class general_agreement_exception : public ValueError_exception {
    using ValueError_exception::ValueError_exception;
};


void register_exeptions(nb::module_ & m);
