...
def get_threads_domdir(sight, offs, shape, wcs, tile_shape=None,
    ...
    asm_full = Assembly.attach(sight, offs)
    ...
    # elevation.
    xi, eta, gamma = quat.decompose_xieta(offs_rep)
    offs_xl = np.array(quat.rotation_xieta(xi, eta, gamma*0 + 90*quat.DEG))
    asm_rep = Assembly.attach(sight, offs_xl)
    ...
    if plot_prefix:
        text = 'Qf=%.2f Uf=%.2f phi=%.1f deg' % (Q/T, U/T, phi / quat.DEG)
