//  Copied from boost/math/quaternion.hpp, with modifications to allow standalone
//  use without the rest of boost.
//
//  History:
//  - 2025-05-15:  copied from original github source and modified.
//
...
class quaternion
{
    public:
        ...
        constexpr void swap(quaternion& o)
        {
        ...
        constexpr quaternion<T> &        operator = (quaternion<T> const & a_affecter)
        {
            a = a_affecter.a;
            b = a_affecter.b;
            c = a_affecter.c;
        ...
        constexpr quaternion<T> &        operator = (quaternion<T> && a_affecter)
        {
        ...
        constexpr quaternion<T> &        operator = (::std::complex<T> const & a_affecter)
        {
            a = a_affecter.real();
            b = a_affecter.imag();
    ...
    private:
        constexpr quaternion<T> &        do_add(T const & rhs, const std::true_type&)
        {
            a += rhs;
            return *this;
        }
        constexpr quaternion<T> &        do_add(T const & rhs, const std::false_type&)
        {
            quaternion<T> result(a + rhs, b, c, d); // exception guard
            swap(result);
            return *this;
        }
        constexpr quaternion<T> &        do_add(std::complex<T> const & rhs, const std::true_type&)
        {
            a += std::real(rhs);
            b += std::imag(rhs);
            return *this;
        }
        constexpr quaternion<T> &        do_add(std::complex<T> const & rhs, const std::false_type&)
        {
        ...
        template <class X>
        constexpr quaternion<T> &        do_add(quaternion<X> const & rhs, const std::true_type&)
        {
            a += rhs.R_component_1();
            b += rhs.R_component_2();
            c += rhs.R_component_3();
        ...
        template <class X>
        constexpr quaternion<T> &        do_add(quaternion<X> const & rhs, const std::false_type&)
        {
        ...
        constexpr quaternion<T> &        do_subtract(T const & rhs, const std::true_type&)
        {
            a -= rhs;
            return *this;
        }
        constexpr quaternion<T> &        do_subtract(T const & rhs, const std::false_type&)
        {
            quaternion<T> result(a - rhs, b, c, d); // exception guard
            swap(result);
            return *this;
        }
        constexpr quaternion<T> &        do_subtract(std::complex<T> const & rhs, const std::true_type&)
        {
            a -= std::real(rhs);
            b -= std::imag(rhs);
            return *this;
        }
        constexpr quaternion<T> &        do_subtract(std::complex<T> const & rhs, const std::false_type&)
        {
            quaternion<T> result(a - std::real(rhs), b - std::imag(rhs), c, d); // exception guard
            swap(result);
            return *this;
        ...
        template <class X>
        constexpr quaternion<T> &        do_subtract(quaternion<X> const & rhs, const std::true_type&)
        {
            a -= rhs.R_component_1();
            b -= rhs.R_component_2();
            c -= rhs.R_component_3();
        ...
        template <class X>
        constexpr quaternion<T> &        do_subtract(quaternion<X> const & rhs, const std::false_type&)
        {
            quaternion<T> result(a - rhs.R_component_1(), b - rhs.R_component_2(), c - rhs.R_component_3(), d - rhs.R_component_4()); // exception guard
            swap(result);
            return *this;
        ...
        constexpr quaternion<T> &        do_multiply(T const & rhs, const std::true_type&)
        {
            a *= rhs;
            b *= rhs;
            c *= rhs;
        ...
        constexpr quaternion<T> &        do_multiply(T const & rhs, const std::false_type&)
        {
        ...
        constexpr quaternion<T> &        do_divide(T const & rhs, const std::true_type&)
        {
            a /= rhs;
            b /= rhs;
            c /= rhs;
        ...
        constexpr quaternion<T> &        do_divide(T const & rhs, const std::false_type&)
        {
            quaternion<T> result(a / rhs, b / rhs, c / rhs, d / rhs); // exception guard
            swap(result);
            return *this;
    ...
    public:
        ...
        constexpr quaternion<T> &        operator += (T const & rhs) { return do_add(rhs, detail::is_trivial_arithmetic_type<T>()); }
        constexpr quaternion<T> &        operator += (::std::complex<T> const & rhs) { return do_add(rhs, detail::is_trivial_arithmetic_type<T>()); }
        template<typename X> constexpr quaternion<T> & operator += (quaternion<X> const & rhs) { return do_add(rhs, detail::is_trivial_arithmetic_type<T>()); }

        constexpr quaternion<T> &        operator -= (T const & rhs) { return do_subtract(rhs, detail::is_trivial_arithmetic_type<T>()); }
        constexpr quaternion<T> &        operator -= (::std::complex<T> const & rhs) { return do_subtract(rhs, detail::is_trivial_arithmetic_type<T>()); }
        template<typename X> constexpr quaternion<T> & operator -= (quaternion<X> const & rhs) { return do_subtract(rhs, detail::is_trivial_arithmetic_type<T>()); }

        constexpr quaternion<T> &        operator *= (T const & rhs) { return do_multiply(rhs, detail::is_trivial_arithmetic_type<T>()); }
        ...
        template<typename X>
        constexpr quaternion<T> &        operator *= (quaternion<X> const & rhs)
        {
        ...
        constexpr quaternion<T> &        operator /= (T const & rhs) { return do_divide(rhs, detail::is_trivial_arithmetic_type<T>()); }
...
// swap:
template <class T>
constexpr void swap(quaternion<T>& a, quaternion<T>& b) { a.swap(b); }
...
template<typename T>
inline constexpr T norm(quaternion<T>const  & q)
{
